...
main_application.title("AE Text Editor")
...
text_editor.config(wrap="word",relief=tk.FLAT)     # if word reach at end it will take whole word to new line

scroll_bar = tk.Scrollbar(main_application)    # it will call the scrollbar class
text_editor.focus_set()                    # it will focus at the starting of cursor
scroll_bar.pack(side=tk.RIGHT,fill=tk.Y)
text_editor.pack(fill=tk.BOTH, expand = True)    
scroll_bar.config(command =text_editor.yview)  # yview means it make vertical scoroll bar
...
def change_font_size(event=None): # we can write anthing in () but for bind we have to wrire something
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        status_bar.config(text=f"Charaters : {characters} Words : {words}")
...
def save_file(event=None):
    ...
    try:
        if url:
            ...
            with open(url,'w',encoding='utf8') as fw:
                fw.write(content)
        else:
        ...
        if text_changed:
            ...
            if mbox is True:                 # it  means YES
                if url:
                    ...
                    url.close()
                    main_application.destroy()
            elif mbox is False:                   # it means NO
                main_application.destroy()
        else:
            main_application.destroy()
    except:
